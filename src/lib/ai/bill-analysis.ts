...
function parseBillAnalysisResponse(rawContent: string): BillAnalysis {
	if (!rawContent) {
		return {
			summary:
				'Bill analysis is unavailable. The model did not return text.',
			potentialIssues: [],
			vendorName: null,
			statementDate: null,
			dueDate: null,
